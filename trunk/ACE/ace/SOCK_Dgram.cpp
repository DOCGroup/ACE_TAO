                      ...
                      int flags) const
{
  ...
  for (i = 0; i < n; i++)
#if ! ((defined(__BORLANDC__) && (__BORLANDC__ >= 0x0530)) || defined(linux))
    // The iov_len is unsigned on Linux and with Borland. If we go ahead
    // and try the if, it will emit a warning.
    if (iov[i].iov_len < 0)
      return -1;
    else
#endif
