...
void
ROA::get_request (CORBA_BOA::dsi_handler handler,
		  ...
		  CORBA_Environment &env)
{
  ...
  // region: the "do_exit" flag, and the "call_count" flag.  These are
  // used in clean shutdown, and can be modified here.
  {
    ACE_MT (ACE_GUARD (ACE_Thread_Mutex, ace_mon, lock_));

    // Applications sometimes make mistakes: here, it'd be polling for
    // a new request after initiating shutdown.
    if (do_exit_)
      {
    ...
    //
    // There are four "threading modes" currently possible with this
    // software, though only two are exposed in the OA API.
    //
  ...
  //
  // If we're "eager", there's not actually any data on that
  // connection yet ... but we still hand it to some other thread as
  // if it were all there.  This gets all the setup out of the
  // critical path so that when data does arrive, its latency is
  // minimal: straight into the IIOP data buffer, in the best (zero
  // copy) case, with no lingering in kernel read queues.
  //
  if (do_thr_create) 
    {
      ...
      // data and hand it over.  That involves no handshaking with the
      // thread we're about to create.  Note that we're also handing
      // off the connection resource too -- when this dispatch context
      // gets destroyed, only then is the connection released.
	...
	return;
      ...
      // in another thread, we must then handle it ourselves.  It's
      // bad news to drop requests, as would happen if we were to just
      // report an exception.
      //
      // XXX this should be logged as some kind of system fault; an
      // administrator would need to deal with these failures if they
      // happen repatedly.
