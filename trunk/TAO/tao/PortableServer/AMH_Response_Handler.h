 ...
 *
 * Class encapsulates state required to send a response back 
 * to the client independent of the thread that originally
 * created the state on the activation-record.  Thus the required state
 * (copied from TAO_Server_Request) is 'stored' on the heap.  
 *
 * One RH is created for every client request and the RH can be used only once
 * i.e.,  the asynchronous method can be called only once.  This class also 
 * encapsulates various initialisation and response-sending functionality that is
 * common to all RHs (generated by the IDL compiler). Thus the IDL-compiler
 * has to generate less code which in turn reduces the overall code size for an 
 * application.
 *
 */
