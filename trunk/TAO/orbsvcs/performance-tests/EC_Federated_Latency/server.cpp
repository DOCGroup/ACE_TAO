...
ACE_RCSID(EC_Federated_Latency, server, "$Id$")
...
{
  ACE_Get_Opt get_opts (argc, argv, "o:p:i:h");
  int c;
  ...
  ACE_TRY_NEW_ENV
    {
      ...
      Servant_var<ECFL_Coordinator> coordinator_impl (
              new ECFL_Coordinator (peer_count,
                                    iterations,
                                    do_dump_history,
                                    orb.in ())
              );
...
template class Servant_var<ECFL_Coordinator>;

#elif defined(ACE_HAS_TEMPLATE_INSTANTIATION_PRAGMA)

#pragma instantiate Servant_var<ECFL_Coordinator>
