...
// This function is not used. Right now the ACE_Flow_Spec is
// constructed with hardcoded values. Once the test works, it will be
// a good idea to replace the hardcoded values with this function.
...
main (int argc, char * argv[])
{
  ...
  // @@ Vishal, this stuff should be handled in the
  // ACE_Object_Manager.  Can you please see if you can remove it here?
...
#define MAXCLIENTS 4
  ...
  // @@ Vishal, can you please make sure to use ACE_DEBUG() and
  // ACE_ERROR macros instead of the printf stuff?
  printf ("Receiver\n");
  ...
  // override this option in the commandline parameters. Is that the
  // right thing to do.
  ...
  if (options.spOptions.bQos)
    {
      if (QOS_IOCTL_SET_BEFORE == options.qosOptions.qosIoctlSet)
        ...
        {
          // The following was the original comment in the windows
          // code. But this does not fall in my path of execution.

          // "this one is tricky.  The intent is to send the RESV
          // after receiving FD_QOS notification.  However, unless we
  ...
  // @@ Vishal, can you please replace this stuff with direct calls to
  // ACE_INET_Addr?  That'll be *much* cleaner!

  ACE_OS::memset ((PVOID) &addr, 0, sizeof addr);
  ...
  // Set IP TTL.
  // @@ Vishal, please replace these calls with the corresponding call
  // to dgram_mcast.set_option().
                         ...
                         ACE_Protocol_Info   *pProtocolInfo)
{
  ...
  if (ret_val != -1)
  ...
  else
    {
      ...
      // @@ Vishal, can you please replace the use of malloc() with operator new?
              ...
              OPTIONS *pOptions)
{
  ...
  *pOptions = default_options;
  ...
  if (!(pOptions->buf = (char *)malloc(pOptions->nBufSize)))
    ...
    {
      // @@ Vishal, can you please see if you can replace the
      // Win32-specific FillMemory() call with a portable
      // ACE_OS::mem*() call?
