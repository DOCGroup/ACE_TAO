...
sub run_node_daemons {
    for ($i = 0; $i < $nr_daemon; ++$i) {
        ...
        $d_cmd = "$DANCE_ROOT/bin/dance_locality_manager";
        $d_param = "-ORBEndpoint $iiop  -u $nodename -o $iorfile -p circular.localityconfig";

        print "Run dance_locality_manager with $d_param\n";
...
# Invoke node daemon.
print "Invoking Locality Manager\n";
$status = run_node_daemons ();

if ($status != 0) {
    print STDERR "ERROR: Unable to execute the locality manager\n";
    kill_open_processes ();
    exit 1;
}
...
print "Start dance_plan_launcher.exe with -x $cdp_file --lm-ior file://$iorfiles[0]\n";
$E = $tg_executor->CreateProcess ("$DANCE_ROOT/bin/dance_plan_launcher",
                        "-x $cdp_file --lm-ior file://$iorfiles[0]");
