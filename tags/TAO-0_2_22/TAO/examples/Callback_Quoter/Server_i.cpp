...
//
// = DESCRIPTION
//    Implementation of the callback quoter Server_i class.
//
// = AUTHOR
...
Server_i::init_naming_service (CORBA::Environment& env)
{
  TAO_TRY
	...
	ACE_ERROR_RETURN ((LM_ERROR,
      ...
      Supplier_var supplier_obj = this->supplier_i_._this (env);
      TAO_CHECK_ENV;

      // Name the object.
      CosNaming::Name supplier_obj_name (1);
      supplier_obj_name.length (1);
      supplier_obj_name[0].id = CORBA::string_dup ("Supplier");

      TAO_CHECK_ENV;

      // Now, attach the object name to the context.
      this->naming_server_->bind (supplier_obj_name,
				  supplier_obj.in (),
				  env);
      TAO_CHECK_ENV;
    }
		...
		CORBA::Environment &TAO_TRY_ENV)
{
  ...
  this->supplier_i_.orb (orb.in ());

  // Activate the servant in the POA.
  CORBA::String_var str  =
    this->orb_manager_.activate_under_child_poa ("Supplier",
						 &this->supplier_i_,
						 TAO_TRY_ENV);
  ACE_DEBUG ((LM_DEBUG,
              "The IOR is: <%s>\n",
