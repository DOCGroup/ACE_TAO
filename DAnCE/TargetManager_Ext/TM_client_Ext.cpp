...
/// Here test the functionality of the CIAO-specific tasks .....

      try
      {
        ::CIAO::Host_Infos_var hostv = targetI_ext ->get_host_cpu ();

        for (CORBA::ULong i = 0; i < hostv->length ();i++)
        {
          ACE_DEBUG ((LM_DEBUG , "The host [%s] takes [%d] cpu \n",
                      hostv[i].hostname.in (),
                      hostv[i].cpu_util
                    ));
        }
      ...
      {
        ex._tao_print_exception ("GET_HOST_CPU::\t\n");
      }

      try
      {
     
        ::CIAO::Host_NodeManager_seq_var nodes = targetI_ext->get_all_node_managers ();

        for (CORBA::ULong i = 0; i < nodes->length ();i++)
        {
          ACE_DEBUG ((LM_DEBUG , "The host [%s] has Node Mgr [%s] \n",
                      nodes[i].host_.in (),
                      orb->object_to_string (nodes[i].node_mgr_)
                    ));
        }
      }
      catch(CORBA::NO_IMPLEMENT &)
      ...
      try
      {
        ...
        CORBA::Long pid = targetI_ext->get_pid ("TargetManager-idd");

        ACE_DEBUG ((LM_DEBUG , "The pid of the component is [%d] \n",
                                pid));
      }
      catch(CORBA::NO_IMPLEMENT &)
      {
