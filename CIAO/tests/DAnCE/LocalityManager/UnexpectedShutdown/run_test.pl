...
$nr_daemon = 2;
@ports = ( 60001, 60002 );
@iorbases = ( "UnexpectedShutdown.ior", "UnexpectedShutdownTwo.ior" );
@iorfiles = 0;
@nodenames = ( "UnexpectedShutdownNode", "UnexpectedShutdownNodeTwo" );
...
sub run_node_daemons {
    for ($i = 0; $i < $nr_daemon; ++$i) {
        ...
        $d_param = "-ORBEndpoint $iiop -s $node_app -n $nodename=$iorfile -t 30 --domain-nc corbaloc:rir:/NameService --node-config nodemanager-besteffort.lconfig";
