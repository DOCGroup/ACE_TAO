...
#define IBP_TIMEOUT_MILLISECONDS 1000
...
namespace SBP{
 ...
 /**
  ...
  bool binSortCriteria (const bin1 &one, const bin1 &two) {
  ...
  int pick_bin_best_fit(std::vector <bin1> & bins, std::vector <item> & items) {
    int b2 = 0;
  ...
  int pick_bin_worst_fit(std::vector <bin1> & /* bins */, std::vector <item> & /* items */) {
    assert ("pick_bin_worst_fit not yet coded." == 0);
    return nbins;
  }

  int pick_bin(std::vector <bin1> & bins, std::vector <item> & items) {
    if (bfwf == 0)
  ...
  int allocate (std::vector <bin1> &bins, std::vector <item> & items) {
    ...
    while (na < nitems) {

      {
        ...
        ACE_Time_Value since = now - start_time;
        if (since.msec() > IBP_TIMEOUT_MILLISECONDS)
          goto done;
      }
    ...
    success++;
        ...
        if (debug) cout << "return allocation (" << success << ")" << endl;
    return success;
  }
  ...
  void print_bin(bin1 &b,  Items &items) {
