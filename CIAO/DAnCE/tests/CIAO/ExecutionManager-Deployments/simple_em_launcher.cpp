...
int ACE_TMAIN (int argc, ACE_TCHAR *argv[])
{
  ...
  try
    {
      ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("*** simple_em_launcher: parsing XML\n")));
      // Parse plan
      ...
      ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("*** simple_em_launcher: resolving execution manager reference.\n")));
      ...
      ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("*** simple_em_launcher: calling prepareplan.\n")));
      Deployment::DomainApplicationManager_var dam = em->preparePlan (*plan,
                                                                    Deployment::ResourceCommitmentManager::_nil ());

      ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("*** simple_em_launcher: calling startLaunch\n")));
      ...
      ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("*** simple_em_launcher: calling finishLaunch\n")));
      da->finishLaunch (conns.in (), false);

      ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("*** simple_em_launcher: calling start\n")));
      da->start ();

      ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("*** simple_em_launcher: start finished, sleeping 5 seconds.\n")));
      ACE_OS::sleep (5);
      ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("*** simple_em_launcher: waking up from sleep, calling destroyApplication\n")));

      dam->destroyApplication (da.in ());

      ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("*** simple_em_launcher: calling destroyManager\n")));

      em->destroyManager (dam.in ());

      ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("*** simple_em_launcher: destroyManager completed.\n")));
