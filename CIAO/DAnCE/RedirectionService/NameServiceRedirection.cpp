...
{
  DANCE_DEBUG (6, (LM_DEBUG, DLINFO ACE_TEXT("NameServiceRedirection::start_binding for node %C plan %C.\n"),
                 node.c_str(),
                 plan.c_str()));
...
{
  DANCE_DEBUG (6, (LM_DEBUG, DLINFO ACE_TEXT("NameServiceRedirection::bind for node %C plan %C component %C and port %C is started.\n"),
                 node.c_str(),
                 plan.c_str(),
                 component.c_str(),
                 port.c_str()));
  ...
  if (0 != this->transactions_.find (key, records))
    {
      DANCE_ERROR (1, (LM_ERROR, DLINFO ACE_TEXT("NameServiceRedirection::bind - Can't find record %C for closing transaction.\n"), key.c_str()));
      return;
    }
  ...
  //    }
  DANCE_DEBUG (6, (LM_DEBUG, DLINFO ACE_TEXT("NameServiceRedirection::bind has finished.\n")));
}
...
{
  DANCE_DEBUG (6, (LM_DEBUG, DLINFO ACE_TEXT("NameServiceRedirection::bind for node %C plan %C and component %C is started.\n"),
                 node.c_str(),
                 plan.c_str(),
                 component.c_str()));
  SRecord record;
  ...
  //    catch(CosNaming::NamingContext::AlreadyBound&)
  //    {
  //        DANCE_DEBUG (6, (LM_DEBUG, "NameServiceRedirection::bind - Already bound exception was thrown. Rebinding\n"));
...
{
  ACE_CString key = node + plan;
  DANCE_DEBUG (6, (LM_DEBUG, DLINFO ACE_TEXT("NameServiceRedirection::finish_binding started for %s.\n"), key.c_str()));
  TRecords* records = 0;
  if (0 != this->transactions_.find (key, records))
    {
      DANCE_ERROR (1, (LM_ERROR, DLINFO ACE_TEXT("NameServiceRedirection::finish_binding - Can't find record %C for closing transaction.\n"), key.c_str()));
      return;
    }
  for (unsigned int i = 0; i < records->size(); i++)
    {
      ...
      try
        ...
        {
          DANCE_DEBUG (6, (LM_DEBUG, DLINFO ACE_TEXT("NameServiceRedirection::finish_bind - Already bound exception was thrown. Rebinding\n")));
  ...
  DANCE_DEBUG (6, (LM_DEBUG, DLINFO ACE_TEXT("NameServiceRedirection::finish_bind has finished.\n")));
}

void
NameServiceRedirection::unbind_context (const ACE_CString& node, const ACE_CString& plan)
{
  DANCE_DEBUG (9, (LM_TRACE, ACE_TEXT("NameServiceRedirection::unbind_context started...\n")));
  ACE_CString kind = "";
  CosNaming::NamingContext_var node_context = this->resolve_context (node, kind, this->naming_.in());
  kind = "DeploymentPlan";
  CosNaming::NamingContext_var plan_context = this->resolve_context (plan, kind, node_context.in());
  DANCE_DEBUG (9, (LM_TRACE, ACE_TEXT("NameServiceRedirection::unbind_context before clear_context for %s.%s\n"), plan.c_str(), kind.c_str()));
  this->clear_context (plan_context.inout());
  CosNaming::Name name (1);
  name.length (1);
  name[0].id = CORBA::string_dup (plan.c_str());
  name[0].kind = CORBA::string_dup (kind.c_str());
  DANCE_DEBUG (9, (LM_TRACE, ACE_TEXT("NameServiceRedirection::unbind_context before unbinding %s.%s\n")
                 , name[0].id.in(), name[0].kind.in()));
  node_context->unbind (name);
  DANCE_DEBUG (9, (LM_TRACE, ACE_TEXT("NameServiceRedirection::unbind_context before destroying plan context.\n")));
  plan_context->destroy();
  DANCE_DEBUG (9, (LM_TRACE, ACE_TEXT("NameServiceRedirection::unbind_context fininshed.\n")));
}
...
{
  DANCE_DEBUG (6, (LM_DEBUG, DLINFO ACE_TEXT("NameServiceRedirection::resolve_context is started for context %C.%C\n"), context_name.c_str(), context_kind.c_str()));
  if (CORBA::is_nil (naming))
    {
      DANCE_DEBUG (6, (LM_DEBUG, DLINFO ACE_TEXT("NameServiceRedirection::resolve_context source context is nil!.\n")));
  ...
  DANCE_DEBUG (6, (LM_DEBUG, DLINFO ACE_TEXT("NameServiceRedirection::resolve_context has finished.\n")));
