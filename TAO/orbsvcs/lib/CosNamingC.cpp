...
// *************************************************************
// class CosNaming::Name
// *************************************************************

// copy constructor
CosNaming::Name::Name (const CosNaming::Name &seq)
	: maximum_ (seq.maximum_),
	  length_ (seq.length_),
	  buffer_ (CosNaming::Name::allocbuf (seq.maximum_)),
...
CosNaming::Name::~Name (void)
{
  ...
  {
    CosNaming::Name::freebuf (this->buffer_);
  }
}

// assignment operator
CosNaming::Name& 
CosNaming::Name::operator= (const CosNaming::Name &seq)
{
  ...
  this->buffer_ = CosNaming::Name::allocbuf (seq.maximum_),
...
static CORBA::TypeCode _tc__tc_CosNaming_Name (CORBA::tk_sequence, sizeof (_oc_CosNaming_Name), (unsigned char *) &_oc_CosNaming_Name, CORBA::B_FALSE);
...
// *************************************************************
// class CosNaming::BindingList
// *************************************************************

// copy constructor
CosNaming::BindingList::BindingList (const CosNaming::BindingList &seq)
	: maximum_ (seq.maximum_),
	  length_ (seq.length_),
	  buffer_ (CosNaming::BindingList::allocbuf (seq.maximum_)),
	  release_ (1) // we always own it
{
  for (CORBA::ULong i=0; i < seq.length_; i++)
  	this->buffer_[i] = seq.buffer_[i];
...
CosNaming::BindingList::~BindingList (void)
{
  if (this->release_) // we own the buffer
  {
    CosNaming::BindingList::freebuf (this->buffer_);
  }
}

// assignment operator
CosNaming::BindingList& 
CosNaming::BindingList::operator= (const CosNaming::BindingList &seq)
{
  if (this == &seq) return *this;
  if (this->release_)
  {
  ...
  this->length_ = seq.length_;
  this->maximum_ = seq.maximum_;
  this->buffer_ = CosNaming::BindingList::allocbuf (seq.maximum_),
  this->release_ =1; // we always own it
  for (CORBA::ULong i=0; i < seq.length_; i++)
  	this->buffer_[i] = seq.buffer_[i];
  return *this;
...
static CORBA::TypeCode _tc__tc_CosNaming_BindingList (CORBA::tk_sequence, sizeof (_oc_CosNaming_BindingList), (unsigned char *) &_oc_CosNaming_BindingList, CORBA::B_FALSE);
