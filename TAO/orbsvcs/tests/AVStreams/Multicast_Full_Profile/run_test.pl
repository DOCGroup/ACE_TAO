...
$multicast_full_profile_server= $EXEPREFIX.".".$DIR_SEPARATOR."server".$EXE_EXT;
$multicast_full_profile_ftp= $EXEPREFIX.".".$DIR_SEPARATOR."ftp".$EXE_EXT;
...
sub server1
{
    ...
    $SVA = Process::Create ($multicast_full_profile_server, $args);
}
...
sub server2
{
    ...
    print ("\nPluggable Server: $args\n");
    $SVB = Process::Create ($multicast_full_profile_server, $args);
}
...
sub ftp
{
    ...
    print ("\nPluggable Ftp: $args\n");
    $CL = Process::Create ($multicast_full_profile_ftp, $args);
}
