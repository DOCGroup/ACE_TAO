...
TAO_NS_LookupManager::TAO_NS_LookupManager (void)
  :app_ (0), activation_manager_ (0)
{
    ACE_DEBUG ((LM_DEBUG, "In Lookup manager %x\n",this));
}
...
void
TAO_NS_LookupManager::_register (TAO_NS_Application_Starter* app)
{
  this->app_ = app;
}

void
TAO_NS_LookupManager::resolve (TAO_NS_Application_Starter*& app)
{
  app = this->app_;
}
...
TAO_NS_LookupManager::resolve (CosNaming::NamingContextExt_var& naming, const char *naming_name ACE_ENV_ARG_DECL)
{
  ...
  naming_ = CosNaming::NamingContextExt::_narrow (naming_obj.in ()
                                                  ACE_ENV_ARG_PARAMETER);
