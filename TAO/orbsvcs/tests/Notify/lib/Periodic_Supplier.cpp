...
{
  TAO_NS_StructuredPushSupplier::connect (proxy, proxy_id ACE_ENV_ARG_PARAMETER);
  ACE_CHECK;

  this->connect_i (ACE_ENV_SINGLE_ARG_PARAMETER);
}

void
TAO_NS_Periodic_Supplier::connect (CosNotifyChannelAdmin::SupplierAdmin_ptr supplier_admin ACE_ENV_ARG_DECL)
{
  TAO_NS_StructuredPushSupplier::connect (supplier_admin ACE_ENV_ARG_PARAMETER);
  ACE_CHECK;
...
void
TAO_NS_Periodic_Supplier::connect_i (ACE_ENV_SINGLE_ARG_DECL)
{
    /// publish
  ...
  added[0].type_name = CORBA::string_dup (this->event_.type ());

  proxy_consumer->offer_change (added, removed ACE_ENV_ARG_PARAMETER);
  ACE_CHECK;

  ACE_DEBUG ((LM_DEBUG, "Supplier offer : (*,%s)\n", this->event_.type ()));*/
}
...
TAO_NS_Periodic_Supplier::svc (void)
{
  ...
  CORBA::Any buffer;
  buffer <<= base_time;
  this->event_.opt_header ("Base Time", buffer);

  buffer <<= this->iter_;
  this->event_.opt_header ("Max Count", buffer);
...
TAO_NS_Periodic_Supplier::offer_change (CosNotification::EventTypeSeq &added, CosNotification::EventTypeSeq& removed ACE_ENV_ARG_DECL)
{
  ...
  ACE_DEBUG ((LM_DEBUG, "Calling Supplier offer change: "));
