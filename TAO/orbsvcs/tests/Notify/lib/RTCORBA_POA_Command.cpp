...
ACE_RCSID(lib, TAO_RTCORBA_POA_Command, "$id$")

#include "tao/debug.h"
#include "LookupManager.h"
#include "Name.h"

TAO_NS_RTCORBA_POA_Command::TAO_NS_RTCORBA_POA_Command (void)
  :command_ (DUMP_STATE), priority_model_ (RTCORBA::CLIENT_PROPAGATED),
   server_priority_ (ACE_DEFAULT_THREAD_PRIORITY), thread_pool_static_threads_ (0), thread_pool_default_priority_ (ACE_DEFAULT_THREAD_PRIORITY)
{
}

TAO_NS_RTCORBA_POA_Command::~TAO_NS_RTCORBA_POA_Command ()
{
}

const char*
TAO_NS_RTCORBA_POA_Command::get_name (void)
{
  return TAO_NS_RTCORBA_POA_Command::name ();
}

const char*
TAO_NS_RTCORBA_POA_Command::name (void)
{
  return TAO_NS_Name.poa_command;
}

void
TAO_NS_RTCORBA_POA_Command::init (ACE_Arg_Shifter& arg_shifter)
{
...
void
TAO_NS_RTCORBA_POA_Command::execute_i (ACE_ENV_SINGLE_ARG_DECL)
{
  if (this->command_ == CREATE)
    {
      ...
      // Create the POA.
      this->create (rt_orb.in (), parent_poa.in () ACE_ENV_SINGLE_ARG_PARAMETER);
      ACE_CHECK;
  ...
  else if (this->command_ == DESTROY)
    {
      ...
      LOOKUP_MANAGER->resolve (poa, this->POA_name_.c_str () ACE_ENV_SINGLE_ARG_PARAMETER);
      ACE_CHECK;
...
void
TAO_NS_RTCORBA_POA_Command::create (RTCORBA::RTORB_ptr rt_orb, PortableServer::POA_ptr parent_poa ACE_ENV_ARG_DECL)
{
  /*
