  ...
  TAO_TRY 
    {
      TAO_PropertySet prop_set;
      {
        ...
        ACE_DEBUG ((LM_DEBUG,
                    "Main : Short s = %d \n",
                    s));
        prop_set.define_property ("short_property", anyval, TAO_TRY_ENV);
        ...
        ACE_DEBUG ((LM_DEBUG,
                    "Main : Long l = %d \n",
                    l));
        prop_set.define_property ("long_property", anyval, TAO_TRY_ENV);
      ...
      }
      {
        ...
        ACE_DEBUG ((LM_DEBUG, "Main: Any holding String \n"));
        ...
        ACE_DEBUG ((LM_DEBUG,
                    ...
                    strvar.in (),
                    newstr));
        prop_set.define_property ("string_property", anyval, TAO_TRY_ENV);
        // Let us test the number of properties now.
        ACE_DEBUG ((LM_DEBUG, 
                    "Main :  Number of props : %d \n",
                    prop_set.get_number_of_properties (TAO_TRY_ENV)));
      ...
      { 
        ...
        if (anyptr != 0)
          ...
          ACE_DEBUG ((LM_DEBUG, "Short property not found\n"));
        // Get the string.
        anyptr = prop_set.get_property_value ("string_property", TAO_TRY_ENV);
        if (anyptr != 0)
          {
            CORBA::String str;
            *anyptr >>= str;
          ...
          ACE_DEBUG ((LM_DEBUG, "string_property not found\n"));
      }
      {
        // Checking "if_property_deifined" of no.
        ACE_DEBUG ((LM_DEBUG,"\nChecking   is_property_defined () \n"));
        if (prop_set.is_property_defined ("short_property", TAO_TRY_ENV) == CORBA::B_FALSE)
          ACE_DEBUG ((LM_DEBUG, "short_property not defined \n"));
        else 
          ACE_DEBUG ((LM_DEBUG, "short_property defined \n"));
        if (prop_set.is_property_defined ("string_property", TAO_TRY_ENV) == CORBA::B_FALSE)
          ACE_DEBUG ((LM_DEBUG, "string_property not defined \n"));
        else
          ACE_DEBUG ((LM_DEBUG, "string_property defined \n"));
      }
      {
        ...
        ACE_DEBUG ((LM_DEBUG, "Length of sequence %d, Maxlength : %d  \n",
                    prop_names.length (),
                    prop_names.maximum ()));
        prop_set.delete_properties (prop_names, TAO_TRY_ENV);
        ACE_DEBUG ((LM_DEBUG, 
                    "Main : Number of props : %d \n",
                    prop_set.get_number_of_properties (TAO_TRY_ENV)));
      ...
      }
      {
        ...
        if (prop_set.delete_all_properties (TAO_TRY_ENV) == CORBA::B_TRUE)
          ACE_DEBUG ((LM_DEBUG, "All properties deleted, I guess \n"));
        else
          ACE_DEBUG ((LM_DEBUG, "delete_all_properties failed \n"));
      ...
      {
        // All the properties are deleted. Lets define a sequence of properties, now
        ...
        // A float property.
        anyval <<= 3.14;
        nproperties[0].property_name = CORBA::string_copy ("float_property");
        nproperties[0].property_value = new CORBA::Any (anyval);
        // A char property.
        anyval <<= '#';
        nproperties[1].property_name = CORBA::string_copy ("char_property");
        nproperties[1].property_value = new CORBA::Any (anyval);
        // A string property.
        anyval <<= "Test string";
        nproperties[2].property_name =  CORBA::string_copy ("string_property");
        nproperties[2].property_value = new CORBA::Any (anyval);

        prop_set.define_properties (nproperties, TAO_TRY_ENV);
      ...
      }
      {
        ...
        TAO_PropertyNamesIterator iterator (prop_set, TAO_TRY_ENV);
        ...
        // Checking reset ().
        ACE_DEBUG ((LM_DEBUG,"Resetting (reset ()) the Iterator, Iterating again :-) \n"));
        iterator.reset (TAO_TRY_ENV);
        while ((ret = iterator.next_one (strvar.out (), TAO_TRY_ENV)) != CORBA::B_FALSE)
          ACE_DEBUG ((LM_DEBUG, "Str : %s \n", strvar.in ()));
        
        // Checking next_n.
        ACE_DEBUG ((LM_DEBUG,"Resetting (reset ()) the Iterator.:-)  \n"));
        iterator.reset (TAO_TRY_ENV);

        CosPropertyService::PropertyNames_var pnames_var;
        ACE_DEBUG ((LM_DEBUG, "Checking next_n (), next 2 \n"));
        CORBA::ULong n = 2;
        if (iterator.next_n (n, pnames_var.out (), TAO_TRY_ENV) == CORBA::B_TRUE)
          ACE_DEBUG ((LM_DEBUG, "Iterator still has items \n"));
        if (pnames_var.ptr () != 0)
          for (size_t i = 0; i < pnames_var.in ().length (); i++)
            ACE_DEBUG ((LM_DEBUG, "str %s \n", (const char *)pnames_var[i]));
        
        ACE_DEBUG ((LM_DEBUG, "next_n (), next 3 \n"));
        n = 3;
        if (iterator.next_n (3, pnames_var.out (), TAO_TRY_ENV) != CORBA::B_TRUE)
          ACE_DEBUG ((LM_DEBUG, "Iterator has no more items \n"));
        if (pnames_var.ptr () != 0)
          for (size_t i = 0; i < pnames_var.in ().length (); i++)
            ACE_DEBUG ((LM_DEBUG, "str %s \n", (const char *)pnames_var[i]));
      }
