...
namespace
{
  ...
  ACE_CString ior_output_filename = "node3_ec.ior";

  Gateway_Initializer::Filename_Array ior_input_files;
}

class Node3_EC : public Kokyu_EC
{
public:
  Node3_EC()
  {
                     ...
                     ))
  {
    ...
    //subt.period = ACE_Time_Value(1,190);
    //subt.phase = ACE_Time_Value(5,168);
...
main (int argc, char* argv[])
{
  ...
  ACE_TRY
    {
#ifdef ACE_HAS_DSUI
      ds_control ds_cntl("Node3_Test_Supplier","node3_enabled.dsui");
#endif //ACE_HAS_DSUI
      ...
      */
      ...
      ACE_DEBUG((LM_DEBUG,"Node3_EC thread %t WORKER_ACTIVATED at %u\n",ACE_OS::gettimeofday().msec()));
      DSTRM_EVENT (MAIN_GROUP_FAM, WORKER_ACTIVATED, 0, 0, NULL);
      //DSTRM_EVENT (WORKER_GROUP_FAM, WORKER_STARTED, m_id, 0, NULL);
      ACE_DEBUG((LM_DEBUG,"Node3_EC thread %t WORKER_STARTED at %u\n",ACE_OS::gettimeofday().msec()));
      ...
      ACE_OS::printf("Node3_EC START at %isec %iusec\n",now.sec(),now.usec());
  ...
  ACE_CATCHANY
    {
      ACE_PRINT_EXCEPTION (ACE_ANY_EXCEPTION, "Node3_EC");
      return 1;
    }
  ACE_ENDTRY;
...
int parse_args (int argc, char *argv[])
{
  ...
  if (ior_input_files.size() == 0)
    {
      ior_input_files.push_back("node4_ec.ior");
    }
