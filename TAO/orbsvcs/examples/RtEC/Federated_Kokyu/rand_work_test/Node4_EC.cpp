...
namespace
{
  ...
  ACE_CString ior_output_filename = "node4_ec.ior";

  Gateway_Initializer::Filename_Array ior_input_files;
}

class Node4_EC : public Kokyu_EC
{
public:
  Node4_EC()
  {
                     ...
                     ))
  {
    ...
    subt.period = ACE_Time_Value(0,627);
    subt.phase = ACE_Time_Value(0,10);
    ...
    subt.period = ACE_Time_Value(0,199);
    subt.phase = ACE_Time_Value(0,80);
    ...
    subt.task_index = 11;
    subt.subtask_index = 1;
    subt.exec = ACE_Time_Value(0,121);
    subt.period = ACE_Time_Value(0,916);
    subt.phase = ACE_Time_Value(0,442);
...
main (int argc, char* argv[])
{
  ...
  ACE_TRY
    {
      ...
      */
      ...
      ACE_DEBUG((LM_DEBUG,"Node4_EC thread %t WORKER_ACTIVATED at %u\n",ACE_OS::gettimeofday().msec()));
      DSTRM_EVENT (MAIN_GROUP_FAM, WORKER_ACTIVATED, 0, 0, NULL);
      //DSTRM_EVENT (WORKER_GROUP_FAM, WORKER_STARTED, m_id, 0, NULL);
      ACE_DEBUG((LM_DEBUG,"Node4_EC thread %t WORKER_STARTED at %u\n",ACE_OS::gettimeofday().msec()));
      ...
      */
      ...
      ACE_OS::printf("Node4_EC START at %isec %iusec\n",now.sec(),now.usec());
  ...
  ACE_CATCHANY
    {
      ACE_PRINT_EXCEPTION (ACE_ANY_EXCEPTION, "Node4_EC");
      return 1;
    }
  ACE_ENDTRY;
...
int parse_args (int argc, char *argv[])
{
  ...
  if (ior_input_files.size() == 0)
    {
      ior_input_files.push_back("node1_ec.ior");
    }
