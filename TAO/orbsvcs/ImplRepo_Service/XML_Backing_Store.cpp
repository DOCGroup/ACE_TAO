...
{
  ACE_TString server_id = ACEXML_escape_string (ACE_TEXT_CHAR_TO_TCHAR(info.server_id.c_str()));
  ACE_TString pname = ACEXML_escape_string (ACE_TEXT_CHAR_TO_TCHAR(info.poa_name.c_str()));
  ACE_TString keyname = ACEXML_escape_string (ACE_TEXT_CHAR_TO_TCHAR(info.key_name_.c_str()));
  ACE_TString altkey = ACE_TEXT("");
  if (!info.alt_info_.null())
    {
      altkey = ACEXML_escape_string (ACE_TEXT_CHAR_TO_TCHAR(info.alt_info_->key_name_.c_str()));
    }

  ACE_TString activator = ACEXML_escape_string (ACE_TEXT_CHAR_TO_TCHAR(info.activator.c_str()));
  ACE_TString cmdline = ACEXML_escape_string (ACE_TEXT_CHAR_TO_TCHAR(info.cmdline.c_str()));
  ACE_TString wdir = ACEXML_escape_string (ACE_TEXT_CHAR_TO_TCHAR(info.dir.c_str()));
  ACE_TString partial_ior = ACEXML_escape_string (ACE_TEXT_CHAR_TO_TCHAR(info.partial_ior.c_str()));
  ACE_TString ior = ACEXML_escape_string (ACE_TEXT_CHAR_TO_TCHAR(info.ior.c_str()));
  ACE_TString amodestr = ACE_TEXT_CHAR_TO_TCHAR(
      ...
      for (CORBA::ULong i = 0; i < elen; ++i)
        {
          ...
          ACE_TString val = ACEXML_escape_string(ACE_TEXT_CHAR_TO_TCHAR(info.env_vars[i].value.in()));
      ...
      for (CORBA::ULong i = 0; i < plen; ++i)
        {
          ...
          ACE_TString name = ACEXML_escape_string(ACE_TEXT_CHAR_TO_TCHAR(info.peers[i].in()));
