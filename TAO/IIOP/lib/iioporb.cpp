...
)
{
	...
	for (cp = (CORBA_String) ACE_OS::strchr ((char *)string, ':') + 1,
	       bytes = cdr.buffer;
	     len--; bytes++) {
...
)
{
    ...
    unsigned char	*buffer = new
			    unsigned char [1 + ACE_OS::strlen ((char *) str) / 2];
    char		*tmp = (char *)str;
    size_t		len = 0;
...
)
{
    ...
    *cp = 0;
    ...
    //
    while ((cp = 
	    ACE_OS::strchr ((char *)data->profile.object_key.buffer, '\\')) != 0) {
...
)
{
    ...
    if (ACE_OS::strncmp ((char *)str, ior_prefix, sizeof ior_prefix - 1) == 0)
