  ...
  if (use_omg_ior_format)
    {
      ...
      if (CDR::encoder (_tc_CORBA_Object, &obj, 0, &cdr, env) != CORBA_TypeCode::TRAVERSE_CONTINUE)
        return 0;
  ...
  else
    {
      ...
      if (obj->QueryInterface (IID_IIOP_Object, (void **)&obj2) != NOERROR)
        {
                      ...
                      CORBA_Environment &env)
{
  ...
  //
  unsigned char	*buffer = new unsigned char [1 + ACE_OS::strlen ((char *) str) / 2];
  char		*tmp = (char *)str;
  size_t		len = 0;
  ...
  if (CDR::decoder (_tc_CORBA_Object, &objref, 0, &stream, env) != CORBA_TypeCode::TRAVERSE_CONTINUE)
    {
      objref = 0;
    }
                       ...
                       CORBA_Environment &env)
{
  ...
  *cp = 0;
  ...
  if ((cp = ACE_OS::strchr ((char *)string, '/')) == 0)
    {
      env.exception (new CORBA_DATA_CONVERSION (COMPLETED_NO));
  ...
  //
  data->profile.object_key.buffer = (unsigned char *) CORBA_string_copy (string);
