...
)
{
    ...
    *(CORBA_Long *)(stream.buffer + 8) =
	...
	writelen = ACE_OS::send (connection, (char _FAR *) buf, buflen, 0);
...
)
{
    ...
    (void) ACE_OS::send (fd, close_message, GIOP_HDR_LEN, 0);
...
send_error (ACE_HANDLE &fd)
{
    ...
    (void) ACE_OS::send (fd, error_message, GIOP_HDR_LEN, 0);
    (void) ACE_OS::shutdown (fd, 2);
    (void) ACE_OS::closesocket (fd);
