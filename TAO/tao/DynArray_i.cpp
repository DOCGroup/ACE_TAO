  ...
  ACE_TRY
    {
      ...
      // so.....
      if (TAO_DynAny_i::unalias (tc, ACE_TRY_ENV) == CORBA::tk_array)
        {
...
TAO_DynArray_i::get_element_type (CORBA::Environment& ACE_TRY_ENV)
{
  ...
  while (element_type->kind (ACE_TRY_ENV) != CORBA::tk_array)
...
TAO_DynArray_i::get_arg_length (CORBA::TypeCode_ptr tc,
                                CORBA::Environment &ACE_TRY_ENV)
{
  while (tc->kind (ACE_TRY_ENV) == CORBA::tk_alias)
    tc = tc->content_type (ACE_TRY_ENV);

  return tc->length (ACE_TRY_ENV);
}
