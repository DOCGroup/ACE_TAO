...
int
TAO_SCIOP_Acceptor::create_profile (const TAO_ObjectKey &object_key,
...
int
TAO_SCIOP_Acceptor::create_new_profile (const TAO_ObjectKey &object_key,
                                       TAO_MProfile &mprofile,
                                       CORBA::Short priority)
{
...
int
TAO_SCIOP_Acceptor::create_shared_profile (const TAO_ObjectKey &object_key,
                                          TAO_MProfile &mprofile,
                                          CORBA::Short priority)
{
  size_t index = 0;
  ...
  if (sciop_profile == 0)
    {
      ...
      code_set_info.ForWcharData.native_code_set =
        TAO_DEFAULT_WCHAR_CODESET_ID;
      sciop_profile->tagged_components ().set_code_sets (code_set_info);
  ...
  if (cdr.read_string (host.out ()) == 0
      || cdr.read_ushort (port) == 0)
    {
      if (TAO_debug_level > 0)
        {
          ACE_DEBUG ((LM_DEBUG,
                      ACE_LIB_TEXT ("TAO (%P|%t) TAO_SCIOP_Acceptor::object_key - ")
