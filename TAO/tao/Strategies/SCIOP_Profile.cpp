...
TAO_SCIOP_Profile::TAO_SCIOP_Profile (const ACE_INET_Addr &addr,
  ...
  : TAO_Profile (TAO_TAG_SCIOP_PROFILE, orb_core, version),
    endpoint_ (addr,
...
int
TAO_SCIOP_Profile::decode (TAO_InputCDR& cdr)
{
  CORBA::ULong encap_len = cdr.length ();

  // Read and verify major, minor versions, ignoring SCIOP profiles
  // whose versions we don't understand.
  CORBA::Octet major = 0, minor = 0;

  if (!(cdr.read_octet (major)
        && major == TAO_DEF_SCIOP_MAJOR
        && cdr.read_octet (minor)
        && minor == TAO_DEF_SCIOP_MINOR))
    {
  ...
  // Unlike IIOP, Tagged Components exist in SCIOR version 1.0!
  ...
  if (cdr.length () != 0 && TAO_debug_level)
    ...
    ACE_DEBUG ((LM_DEBUG,
                ACE_TEXT ("%d bytes out of %d left after SCIOP profile data\n"),
                cdr.length (),
                encap_len));
...
void
TAO_SCIOP_Profile::parse_string (const char *ior
                                ACE_ENV_ARG_DECL)
{
  if (!ior || !*ior)
    {
  ...
  if (this->version_.major != TAO_DEF_SCIOP_MAJOR ||
      this->version_.minor >  TAO_DEF_SCIOP_MINOR)
    {
      ACE_THROW (CORBA::INV_OBJREF (
                   CORBA_SystemException::_tao_minor_code (
                     TAO_DEFAULT_MINOR_CODE,
...
int
TAO_SCIOP_Profile::encode (TAO_OutputCDR &stream) const
{
...
IOP::TaggedProfile &
TAO_SCIOP_Profile::create_tagged_profile (void)
{
  ...
  if (this->tagged_profile_.profile_data.get_buffer () == 0)
    {
      ...
      this->tagged_profile_.tag = TAO_TAG_SCIOP_PROFILE;
