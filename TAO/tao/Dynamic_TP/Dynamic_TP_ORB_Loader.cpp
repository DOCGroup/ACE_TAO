...
TAO_Dynamic_TP_ORB_Loader::init (int argc, ACE_TCHAR* argv[])
{
  ...
  TAO_RT_ORBInitializer::TAO_DynamicTP_DT_LifeSpan lifespan = TAO_RT_ORBInitializer::TAO_DynamicTP_DT_INFINITIVE;
      ...
      if (0 != (current_arg = arg_shifter.get_the_parameter
                  (ACE_TEXT("-ORBPriorityMapping"))))
        {
          ...
          if (ACE_OS::strcasecmp (name,
            ...
            ACE_DEBUG ((LM_DEBUG,
                        ACE_TEXT("Dynamic_TP_ORB_Loader - unknown argument")
                                   ...
                                   (ACE_TEXT("-RTORBDynamicThreadIdleTimeout"))))
        {
          ...
          lifespan = TAO_RT_ORBInitializer::TAO_DynamicTP_DT_IDLE;
          arg_shifter.consume_arg ();
        }
      else if (0 != (current_arg = arg_shifter.get_the_parameter
                                   (ACE_TEXT("-RTORBDynamicThreadRunTime"))))
        {
          ...
          int timeout = ACE_OS::atoi (name);
          dynamic_thread_time = ACE_Time_Value (0, timeout);
          lifespan = TAO_RT_ORBInitializer::TAO_DynamicTP_DT_FIXED;
          arg_shifter.consume_arg ();
        }
    else
      {
        ...
        {
          ACE_ERROR ((LM_ERROR,
                      ACE_TEXT("Dynamic_TP_ORB_Loader: Unknown option ")
                      ACE_TEXT("<%s>.\n"),
                      argv[curarg]));
        }
      }
...
ACE_FACTORY_DEFINE (TAO_DynamicTP_ORB, TAO_Dynamic_TP_ORB_Loader)
