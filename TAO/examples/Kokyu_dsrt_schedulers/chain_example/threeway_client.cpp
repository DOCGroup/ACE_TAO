...
{
  ds_control ds_cntrl ("DT_Chain_Client", "chain_enable.dsui");
...
/*      ORBSVCS_Time::Time_Value_to_TimeT (sched_param.deadline,
					 ...
					 ACE_Time_Value (sec_left, usec_left));
*/
      ORBSVCS_Time::Time_Value_to_TimeT (sched_param.deadline,
                                         ACE_OS::gettimeofday () +
                                         ACE_Time_Value (sec_period, usec_period));
  ...
  for(int i=0;i<niteration_;i++)
    {
      if(i>0 && enable_dynamic_scheduling)
        {
//          sched_param.deadline = sched_param.deadline+period_*10000-left_work*10000;
