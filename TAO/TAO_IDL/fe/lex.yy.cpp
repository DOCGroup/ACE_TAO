  ...
  if (!is_real_filename)
    {
      ...
      // I don't see the benefit of using ->compare since this is
      // targeted at IDL identifiers not at filenames and in the case
      // of OpenVMS (case-insensitive filesystem) gets really
      // problematic as filenames retrieved through different
      // mechanisms may give different casing.
      is_main_filename =
        idl_global->path_cmp (idl_global->main_filename ()->get_string (),
                              full_fname) == 0;
#else
      is_main_filename 
