...
{
  ACE_Get_Opt get_opts (argc, argv, ACE_TEXT ("k:l:t:x:n:d"));
  int c;
  while ((c = get_opts ()) != -1)
    switch (c)
      {
      ...
      default:
        ACE_ERROR_RETURN ((LM_ERROR,
            ACE_TEXT ("(%P|%t) Usage: client -k <ior> [-l <ior>] [-t <sec>] [-x <sec>] [-n <N>] [-d]\n")),
            -1);
      }
...
tc1_basic_idle_close (CORBA::ORB_ptr orb, Test::Echo_ptr echo, Test::Echo_ptr echo2)
{
  ...
  // Two one-second waits (nested call and post-reply wait) remain below Y=3.
  // Both server-1 connections and the server-2 connection remain active.
  ok &= echo->ping (0, 1, echo2, 1, 2, 1);

  ok &= check ("TC-1 after nested call (expect 1)", cache_size(orb), 1);
...
// ---------------------------------------------------------------------------
static bool
tc3_activity_on_reuse (CORBA::ORB_ptr orb, Test::Echo_ptr echo, Test::Echo_ptr echo2)
{
  ACE_DEBUG ((LM_INFO,
  ...
  try
    {
      ...
      if (ior2)
        {
          ACE_DEBUG ((LM_INFO, ACE_TEXT ("(%P|%t) Client received echo2\n")));
      ...
      if (disabled_tc)
      ...
      else
        {
          ...
          all_pass &= tc3_activity_on_reuse (orb.in (), echo.in (), echo2.in ());
        }
      ...
      if (!CORBA::is_nil (echo2.in ()))
        {
          ACE_DEBUG ((LM_INFO, ACE_TEXT ("(%P|%t) Shutting down echo2\n")));
          echo2->shutdown ();
        }
