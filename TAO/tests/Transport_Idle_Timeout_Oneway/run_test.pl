...
my $CL = $client->CreateProcess (
    "client",
    "-ORBSvcConf client.conf -k file://$client_ior -ORBDebugLevel $cdebug_level -ORBVerboseLogging 1"
);
...
my $client_status = $CL->SpawnWaitKill ($client->ProcessStartWaitInterval () + 10);
if ($client_status != 0) {
