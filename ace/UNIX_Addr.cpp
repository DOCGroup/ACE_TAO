...
ACE_UNIX_Addr::set_addr (void *addr, int len)
{
  ...
  ACE_OS_String::memcpy ((void *) &this->unix_addr_,
                         (void *) addr, 
                         len);
}
...
{
  (void) ACE_OS_String::memset ((void *) &this->unix_addr_,
                                0,
...
ACE_UNIX_Addr::set (const ACE_UNIX_Addr &sa)
{
  if (sa.get_type () == AF_ANY)
    ...
    ACE_OS_String::strcpy (this->unix_addr_.sun_path,
...
{
  (void) ACE_OS_String::memset ((void *) &this->unix_addr_, 0, 
                                sizeof this->unix_addr_);
  this->unix_addr_.sun_family = AF_UNIX;
  ACE_OS_String::strcpy (this->unix_addr_.sun_path, un->sun_path);
...
ACE_UNIX_Addr::set (const char rendezvous_point[])
{
  (void) ACE_OS_String::memset ((void *) &this->unix_addr_, 
			 ...
			 sizeof this->unix_addr_);
  this->unix_addr_.sun_family = AF_UNIX;
  size_t len = ACE_OS_String::strlen (rendezvous_point);
  size_t maxlen = sizeof this->unix_addr_.sun_path;
 
  (void) ACE_OS_String::memcpy (this->unix_addr_.sun_path, 
                                rendezvous_point,
  ...
  this->ACE_Addr::base_set (AF_UNIX, 
			    ...
			    ACE_OS_String::strlen (this->unix_addr_.sun_path));
  return 0;
}
