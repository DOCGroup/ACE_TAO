                                  ...
                                  const ACE_TCHAR *persistent_file)
{
  ...
  if (this->config_->open_section (config_->root_section (),
				   "htbp", 1,
                                   this->htbp_key_) != 0)
    ACE_ERROR_RETURN ((LM_ERROR,
                       "ACE::HTBP::Environment::initialize (). "
                       "Open HTBP Section failed"),
                      -1);
  return 0;
}
...
int
ACE::HTBP::Environment::get_htid_url (ACE_CString &htid_url) const
{
...
int
ACE::HTBP::Environment::set_htid_url (const char *htid_generator_url)
{
...
int
ACE::HTBP::Environment::get_proxy_host (ACE_CString &proxy_host) const
{
  return this->config_->get_string_value (this->htbp_key_,
                                          "proxy_host",
                                          proxy_host);
...
int
ACE::HTBP::Environment::set_proxy_host (const char *proxy_host)
{
  return this->config_->set_string_value (this->htbp_key_,
                                          "proxy_host",
                                          proxy_host);
