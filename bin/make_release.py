...
def update_spec_file ():
    ...
    spec_file = open (doc_root + "/ACE/rpmbuild/ace-tao.spec", 'r+')
    new_spec = ""
...
#        print prev_tao_var
    ...
    control_file = open (doc_root + "/ACE/debian/control", 'r+')
    new_ctrl = ""
...
""" % (comp_versions["ACE_version"], comp_versions["TAO_version"], comp_versions["ACE_version"])
    if opts.take_action:
        dsc_file = open (doc_root + "/ACE/debian/ace.dsc", 'r+')
        dsc_file.seek (0)
        dsc_file.truncate (0)
...
def tag ():
    ...
    if opts.release_type == "fix":
        ...
        # Tag ACE
        svn_client.copy (opts.repo_root + "/branches/Riverace-6.1",
...
def generate_workspaces (stage_dir):
    ...
    static_vc10_option = ' -static -name_modifier *_vc10_Static -apply_project -exclude TAO/CIAO '
    ...
    ex (mpc_command + " -type vc9 " + static_vc9_option + workers_option + redirect_option)

    print "\tGenerating VC10 Static solutions"
    ex (mpc_command + " -type vc10 " + static_vc10_option + workers_option + redirect_option)
