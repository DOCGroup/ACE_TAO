...
int test(ACE_Configuration* config)
{
...
int test_io(ACE_Configuration* config)
{
...
int main(int argc, char** argv)
{
  ...
  HKEY root = ACE_Configuration_Win32Registry::resolve_key(HKEY_LOCAL_MACHINE, "Software\\ACE\\test");
  if(!root)
  {
    return -1;
  }
  ...
  }
  {
    ...
    if(result)
    {
      ACE_DEBUG((LM_ERROR, "Heap Configuration test failed (%d)\n", result));
      return -1;
    }
  }
  ...
  }
  {
    ...
    if(result)
    {
      ACE_DEBUG((LM_ERROR, "Persistent Heap Configuration test failed (%d)\n", result));
      return -1;
    }
  }
  delete pers_config;
