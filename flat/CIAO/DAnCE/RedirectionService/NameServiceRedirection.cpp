...
{
  ACE_DEBUG ( (LM_DEBUG, "[%M] NameServiceRedirection::start_binding for node %s plan %s.\n",
               node.c_str(),
               plan.c_str()));
...
{
  ACE_DEBUG ( (LM_DEBUG, "[%M] NameServiceRedirection::bind for node %s plan %s component %s and port %s is started.\n",
               node.c_str(),
               plan.c_str(),
               component.c_str(),
               port.c_str()));
  ...
  if (0 != this->transactions_.find (key, records))
    {
      ACE_ERROR ( (LM_ERROR, "[%M] NameServiceRedirection::bind - Cann't find record %C for closing transaction.\n", key.c_str()));
      return;
    }
...
//    }
  ACE_DEBUG ( (LM_DEBUG, "[%M] NameServiceRedirection::bind has finished.\n"));
}
...
{
  ACE_DEBUG ( (LM_DEBUG, "[%M] NameServiceRedirection::bind for node %s plan %s and component %s  is started.\n",
               node.c_str(),
               plan.c_str(),
               component.c_str()));
  SRecord record;
  ...
  if (0 != this->transactions_.find (key, records))
    {
      ACE_ERROR ( (LM_ERROR, "[%M] NameServiceRedirection::bind - Cann't find record %s for closing transaction.\n", key.c_str()));
      return;
    }
...
//    catch(CosNaming::NamingContext::AlreadyBound&)
//    {
//        ACE_DEBUG((LM_DEBUG, "[%M] NameServiceRedirection::bind - Already bound exception was thrown. Rebinding\n"));
...
{
  ACE_CString key = node + plan;
  ACE_DEBUG ( (LM_DEBUG, "[%M] NameServiceRedirection::finish_binding started for %s.\n", key.c_str()));
  TRecords* records = 0;
  if (0 != this->transactions_.find (key, records))
    {
      ACE_ERROR ( (LM_ERROR, "[%M] NameServiceRedirection::finish_binding - Cann't find record %s for closing transaction.\n", key.c_str()));
      return;
    }
  for (unsigned int i = 0; i < records->size(); i++)
    {
      ...
      try
        ...
        {
          ACE_DEBUG ( (LM_DEBUG, "[%M] NameServiceRedirection::finish_bind - Already bound exception was thrown. Rebinding\n"));
  ...
  ACE_DEBUG ( (LM_DEBUG, "[%M] NameServiceRedirection::finish_bind has finished.\n"));
}

void
NameServiceRedirection::unbind_context (const ACE_CString& node, const ACE_CString& plan)
{
  ACE_DEBUG ( (LM_TRACE, "[%M] NameServiceRedirection::unbind_context started...\n"));
  ACE_CString kind = "";
  CosNaming::NamingContext_var node_context = this->resolve_context (node, kind, this->naming_.in());
  kind = "DeploymentPlan";
  CosNaming::NamingContext_var plan_context = this->resolve_context (plan, kind, node_context.in());
  ACE_DEBUG ( (LM_TRACE, "[%M] NameServiceRedirection::unbind_context before clear_context for %s.%s\n", plan.c_str(), kind.c_str()));
  this->clear_context (plan_context.inout());
  CosNaming::Name name (1);
  name.length (1);
  name[0].id = CORBA::string_dup (plan.c_str());
  name[0].kind = CORBA::string_dup (kind.c_str());
  ACE_DEBUG ( (LM_TRACE, "[%M] NameServiceRedirection::unbind_context before unbinding %s.%s\n"
      , name[0].id.in(), name[0].kind.in()));
  node_context->unbind (name);
  ACE_DEBUG ( (LM_TRACE, "[%M] NameServiceRedirection::unbind_context before destroying plan context.\n"));
  plan_context->destroy();
  ACE_DEBUG ( (LM_TRACE, "[%M] NameServiceRedirection::unbind_context fininshed.\n"));
}
...
{
  ACE_DEBUG ( (LM_DEBUG, "[%M] NameServiceRedirection::resolve_context is started for context %s.%s\n", context_name.c_str(), context_kind.c_str()));
  if (CORBA::is_nil (naming))
    {
      ACE_DEBUG ( (LM_DEBUG, "[%M] NameServiceRedirection::resolve_context source context is nil!.\n"));
  ...
  ACE_DEBUG ( (LM_DEBUG, "[%M] NameServiceRedirection::resolve_context has finished.\n"));
