...
int ACE_TMAIN (int argc, ACE_TCHAR *argv[])
{
  ...
  try
    {
      DANCE_DEBUG ((LM_DEBUG, "*** simple_nm_launcher: parsing XML\n"));
      // Parse plan
      ...
      DANCE_DEBUG ((LM_DEBUG, "*** simple_nm_launcher: resoling node manager reference.\n"));
      CORBA::Object_var obj = orb->string_to_object (argv[1]);
      Deployment::NodeManager_var nm = Deployment::NodeManager::_narrow (obj.in ());
      
      if (CORBA::is_nil (nm.in ()))
        {
          DANCE_ERROR ((LM_ERROR, "*** simple_nm_launcher: NodeManager reference is nil."));
          throw 1;
        }

      DANCE_DEBUG ((LM_DEBUG, "*** simple_nm_launcher: calling prepareplan.\n"));
      Deployment::NodeApplicationManager_var nam = nm->preparePlan (*plan,
                                                                    Deployment::ResourceCommitmentManager::_nil ());
      
      DANCE_DEBUG ((LM_DEBUG, "*** simple_nm_launcher: calling startLaunch\n"));
      ...
      DANCE_DEBUG ((LM_DEBUG, "*** simple_nm_launcher: calling finishLaunch\n"));
      na->finishLaunch (conns.in (), false);
      
      DANCE_DEBUG ((LM_DEBUG, "*** simple_nm_launcher: calling start\n"));
      na->start ();
      
      DANCE_DEBUG ((LM_DEBUG, "*** simple_nm_launcher: start finished, sleeping 5 seconds.\n"));
      ACE_OS::sleep (5);
      DANCE_DEBUG ((LM_DEBUG, "*** simple_nm_launcher: waking up from sleep, calling destroyApplication\n"));
      
      nam->destroyApplication (na.in ());
      
      DANCE_DEBUG ((LM_DEBUG, "*** simple_nm_launcher: calling destroyManager\n"));
      
      nm->destroyManager (nam.in ());
      
      DANCE_DEBUG ((LM_DEBUG, "*** simple_nm_launcher: destroyManager completed.\n"));
      
      orb->destroy ();
    }
  catch (Deployment::StopError &ex)
    {
      DANCE_ERROR ((LM_ERROR, "*** Caught StopError exception with name %s and reason %s\n",
                  ex.name.in (), ex.reason.in ()));
      return -1;
    }
  catch (Deployment::StartError &ex)
    {
      DANCE_ERROR ((LM_ERROR, "*** Caught StartError exception with name %s and reason %s\n",
                  ex.name.in (), ex.reason.in ()));
      return -1;
    }
  catch (CORBA::Exception &ex)
    {
      DANCE_ERROR ((LM_ERROR, "*** Caught CORBA exception: %s\n",
                  ex._info ().c_str ()));
      return -1;
