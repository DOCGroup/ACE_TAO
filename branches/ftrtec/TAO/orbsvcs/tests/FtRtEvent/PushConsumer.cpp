                         ...
                         ACE_THROW_SPEC ((CORBA::SystemException))
{
  ...
  if (event.length() >0) {
    /// there is at least one message inside the event
    ...
    if ( num_iterations_ > static_cast<int>(x) &&
      ...
      /// the sequence number of receivered event exceed num_iterations_ (which is specified in command line) and
      /// the the number of events received is less than num_events_to_end_ 
        if (x < run_times_.size())
          // record the elaps time for the event
          run_times_[x] = static_cast<int>(elaps/10);
      }
    else {
      // end condition satified, close the application.
      ACE_TRY {
