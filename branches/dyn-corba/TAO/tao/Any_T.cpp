                           ...
                           const T *& _tao_elem)
{
  ...
  ACE_TRY_NEW_ENV
    {
      CORBA::TypeCode_ptr any_tc = any._tao_get_typecode ();
      CORBA::TypeCode_ptr tmp = any_tc;

      CORBA::TCKind any_kind = any_tc->kind (ACE_ENV_SINGLE_ARG_PARAMETER);
      ACE_TRY_CHECK;

      while (any_kind == CORBA::tk_alias)
        {
          tmp = tmp->content_type (ACE_ENV_SINGLE_ARG_PARAMETER);
          ACE_TRY_CHECK;

          any_kind = tmp->kind (ACE_ENV_SINGLE_ARG_PARAMETER);
          ACE_TRY_CHECK;
        }
