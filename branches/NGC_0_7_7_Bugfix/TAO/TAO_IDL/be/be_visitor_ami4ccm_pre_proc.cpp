...
int
be_visitor_ami4ccm_pre_proc::visit_root (be_root *node)
{
  if (this->visit_scope (node) == -1)
    {
      ACE_ERROR_RETURN ((LM_ERROR,
...
int
be_visitor_ami4ccm_pre_proc::visit_module (be_module *node)
{
...
be_visitor_ami4ccm_pre_proc::visit_uses (be_uses *node)
{
  ...
  AST_Component *component =
    AST_Component::narrow_from_scope (node->defined_in ());
  ...
  if (uses)
    {
      uses->set_defined_in (node->defined_in ());
...
return 0;
  ...
  if (sendc_interface)
    {
      sendc_interface->set_defined_in (node->defined_in ());
          ...
          if (!d)
            {
              ACE_ERROR_RETURN ((
                  LM_ERROR,
                  "(%N:%l) be_visitor_ami4ccm_pre_proc::create_sendc_method - "
                  "bad node in this scope\n"
                ),
                0
              );
...
int
be_visitor_ami4ccm_pre_proc::generate_name (ACE_CString &destination,
                                        const char *prefix,
