...
{
  if (x != 0)
    return this->write_string (ACE_OS_String::strlen(x), x);
...
{
  if (x != 0)
    return this->write_wstring (ACE_OS_String::wslen (x), x);
...
operator<< (ACE_OutputCDR &os, ACE_OutputCDR::from_wstring x)
{
  ACE_CDR::ULong len = 0;;
  if (x.val_ != 0)
    len = ACE_OS_String::wslen (x.val_);
...
operator>> (ACE_InputCDR &is, ACE_InputCDR::to_wstring x)
{
  ...
  // check if the bounds are satisfied
  return (is.good_bit () &&
	  (ACE_OS_String::wslen (x.val_) <= x.bound_));
}
