...
ACE_RCSID(EC_Federated_Latency, Loopback_Supplier, "$Id$")

ECFL_Loopback_Supplier::ECFL_Loopback_Supplier (CORBA::Long experiment_id)
                                 ...
                                 TAO_ENV_ARG_DECL)
{
  ...
  sh0.type   = ECFL_RESPONSE_EVENT_TYPE;
  ...
  for (CORBA::ULong i = 0; i != events.length (); ++i)
    {
      ...
      events[i].header.type = ECFL_RESPONSE_EVENT_TYPE;
