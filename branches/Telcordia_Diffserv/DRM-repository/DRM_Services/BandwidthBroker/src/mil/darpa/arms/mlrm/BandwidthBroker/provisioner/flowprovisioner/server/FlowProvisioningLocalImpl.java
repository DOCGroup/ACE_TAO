// $Id: FlowProvisioningLocalImpl.java
 ...
 */
public class FlowProvisioningLocalImpl {
  ...
  static {
	  ...
	  try {
			serverProps.load(FlowProvisioningLocalImpl.class.getResourceAsStream("/ioslog.prop"));
  ...
  static Logger logger =Logger.getLogger(FlowProvisioningLocalImpl.class.getName());
  ...
  public FlowProvisioningLocalImpl()
  {
	  System.err.println("In FP Local Impl Constructor");
  ...
  // node passwords etc.  - added MRC for LocalImpl
  public void initAuth(String file, String decryptKey) {
    byte[] buf = new byte[8];
    byte[] b = new byte[100];
    int len;
    ...
    try {
      ...
      //in.read(buf); 
      buf = decryptKey.getBytes();
      SecretKeySpec key = new SecretKeySpec (buf, "DES");
      Cipher c = Cipher.getInstance("DES");
      c.init(Cipher.DECRYPT_MODE, key);
      
      CipherInputStream read_back = new CipherInputStream(getClass().getResourceAsStream(file), c);
      ...
      while (st.countTokens() >= 5) {
        ...
        node = new FlowProvisioningNodeInfo(nodeName, st.nextToken(), st.nextToken(), 
...
*/
  ...
  private void addLinuxFlow(FlowProvisioningNodeInfo node,
			    ...
			    int rate,
			    int burst,
			    FlowInfo filter) throws IOException
  {
    String cmds;
  ...
  public void addFlow (String nodeName,
    ...
    System.err.println("Adding Flow: " + flowID);
