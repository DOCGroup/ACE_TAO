 ...
 *
 * @brief Thread-safe refounting, calls the <release> method when
 *        refcount falls to 0.
 *
 * The refcount is initialized to 0. When an instance of a derived
 * class becomes owned  by a managed pointer (Refcountable_Guard_T)
 * the reference count becomes non-zero. A debug message is printed
 * from the destructor if a refcounted is deleted with a non-zero
 * refcount.
 *
 * Instances declared on the stack should always have a refcount of
 * zero.
 *
 * A method that creates or simply returns an instance of Refcountable
 * should not increment the reference count. It is the responsibility
 * of the client to increment the reference count (take ownership or
 * guard against deletion). The client cannot know if the method will
 * or will not incr the refcount in its behalf.
 *
