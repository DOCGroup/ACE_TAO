...
//      if (TAO_debug_level > 0)
  //      {
    //      ACE_DEBUG ((LM_DEBUG,
      //                ACE_TEXT ("TAO (%P|%t) NIOP connection failed.\n")
...
//    NIOP_endpoint->object_addr ();
  ...
  TAO_NIOP_Connection_Handler *svc_handler = 0;

//  if (svc_handler_table_.find (remote_address, svc_handler) == -1)
    {
      TAO_NIOP_Connection_Handler *svc_handler_i = 0;
      ACE_NEW_RETURN (svc_handler_i,
                      TAO_NIOP_Connection_Handler (this->orb_core (),
                                                   this->lite_flag_),
                      0);
...
//      if (TAO_debug_level > 2)
  //      ACE_DEBUG ((LM_DEBUG,
    //                ACE_TEXT ("(%P|%t) NIOP_Connector::connect - ")
