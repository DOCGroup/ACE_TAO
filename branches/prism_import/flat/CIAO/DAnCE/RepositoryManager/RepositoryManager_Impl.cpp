                                                     ...
                                                     )
{
  ...
  {
    ACE_ERROR((LM_ERROR, "[%M] [RM] problem updating the PackageConfiguration!\n"));
  ...
  )
{
  ...
  ACE_DEBUG((LM_DEBUG, "[%M] The number of types: %d\n", num_entries));
    ...
    if (supportedTypes.length () != 0)
    {
      ...
      for (CORBA::ULong i = 0; i < len; ++i)
      {
        if (this->types_.unbind (ACE_CString (supportedTypes[i]), ACE_CString (name)) != 0)
          ACE_DEBUG((LM_DEBUG, "[%M] Could not find type %s with package name %s!\n",
...
void CIAO_RepositoryManagerDaemon_i::dump (void)
{
  ...
  ACE_DEBUG((LM_DEBUG, "[%M] NAMES:\n"));
  this->names_.dump ();
  ACE_DEBUG((LM_DEBUG, "[%M] UUIDs:\n"));
  this->uuids_.dump ();
  ACE_DEBUG((LM_DEBUG, "[%M] Component Interface Types:\n"));
  this->types_.dump ();
