...
DROP TABLE IF EXISTS Router, Switch, L3Interface, L2Interface,PTP, FaultSubscriptions, NetworkMonitorEventSubscriptions;
...
CREATE TABLE IF NOT EXISTS L3Interface (
  ...
  vlanID         INT UNSIGNED        default 0,	
  ptpID         INT UNSIGNED        default 0,	
  linkID	INT UNSIGNED	REFERENCES Link (id),
...
CREATE TABLE IF NOT EXISTS FlowRecord (
  ...
  l3PathID	 INT UNSIGNED	NOT NULL REFERENCES L3Path (id),
...
CREATE TABLE IF NOT EXISTS L3Hops (
  ...
  isOldPath      BOOL	default '0',
  UNIQUE KEY (id, hopNumber, l3PathID, isOldPath),
  UNIQUE KEY (id, l3InterfaceID, l3PathID, isOldPath),
  PRIMARY KEY (id)
) ENGINE = NDBCLUSTER;
