...
int send_datagram( ACE_SOCK_Dgram_Bcast &socket, int datagram_no)
{
    ...
    if ( socket.send( dgram_buffer, sizeof( dgram_buffer ), dgram_port ) < 0 )
        ACE_ERROR_RETURN(( LM_ERROR, "%p\n", "Cannot broadcast datagram"), -1);
    else
        ACE_DEBUG((LM_INFO,"%s sent\n",dgram_buffer));
    return 0;
}
...
int run_sender( )
{
    ACE_DEBUG((LM_INFO,"Sending %d datagrams on port %d\n", dgrams_no, dgram_port));
    ...
    ACE_ERROR_RETURN(( LM_ERROR, "%p\n", "Cannot open broadcast socket"  ), -1);
}
...
*/int run_receiver( )
{
    ACE_DEBUG((LM_INFO,"Receiving datagrams from port %d with timeout %d ms\n",
    ...
    if ( socket.open( ACE_INET_Addr( dgram_port ) ) != -1 )
        ...
        {
            ACE_DEBUG(( LM_INFO, "%s received\n", dgram_buffer ));
            return 0;
        }
        else
            ACE_ERROR_RETURN(( LM_ERROR, "%p\n", "Cannot receive datagrams"  ), -1);
...
Function sends a number of datagrams, spwans child thread or process and
...
int run_auto_test( const ACE_TCHAR *prog_name)
{
    ...
    ACE_DEBUG((LM_INFO,"Running auto_tests in process mode\n"));
    ...
    ACE_DEBUG((LM_INFO,"Running auto_tests in thread mode\n"));
    if ( ACE_Thread_Manager::instance ()->spawn ( run_thread_receiver ) == -1)
        ACE_ERROR_RETURN((LM_ERROR, ACE_TEXT ("%p\n"), ACE_TEXT ("spawn_n ()")), -1);
#else
    ACE_ERROR_RETURN(( LM_ERROR,
                         "Cannot run in auto_test mode without fork or threads.\n"),
        -1);
#endif /* !defined( ACE_LACKS_FORK )*/

    ACE_DEBUG((LM_INFO,"Sending datagrams on port %d in auto_test mode\n",
                  dgram_port));
    ...
    if ( socket.open( ACE_Addr::sap_any) != -1 )
    {
        ...
        while( 1 )
        {
            ...
            if ( ACE_Process_Manager::instance ()->wait( child_pid,
                     child_timeout, &receiver_exit_code ) == child_pid )
                break;
#else /* ACE_HAS_THREAD */
        ...
        socket.close();
        ACE_DEBUG(( LM_INFO, "Child finished with %d exit code\n", receiver_exit_code ));
    }
    else
        ACE_ERROR_RETURN(( LM_ERROR, "%p\n", "Cannot open broadcast socket"  ), -1);
